...
def get_pos_from_toks(toks, tokenizer, nlp=None):
    ...
    return [get_most_frequent_item(pos[prefix_len - tok_len:prefix_len]) for tok_len, prefix_len in
...
def weighted_lm_loss(model, input_ids, target_ids, attention_mask, weights=None, ignore_index=-100):
...
def weighted_lm_loss_with_outputs(outputs, input_ids, target_ids, weights=None, ignore_index=-100):
    loss_fn = nn.CrossEntropyLoss(ignore_index=ignore_index, reduction='none')
    batch_size = len(input_ids)
    reshaped_logits = outputs.logits[:, :-1, :].reshape(-1, outputs.logits.shape[-1])
    reshaped_labels = target_ids[:, 1:].reshape(-1)
...
def create_colored_text(words: List[str], data: List[float], font_path='DejaVuSansMono.ttf', pos_cmap=None,
                        neg_cmap=None, max_intensity=.8) -> Image:
